...
class DB {
  ...
  static async update_product_info(
    product_id,
    name,
    description,
    image_url,
    ...
    await db.query(
      "UPDATE Products SET name = ?, description = ?, image_url = ?, price = ?, weight = ?, quantity = ? WHERE product_id = ?",
      [name, description, image_url, price, weight, quantity]
    );
  }
  ...
  static async set_product_categories(product_id, category_ids) {
    ...
    await db.query("DELETE FROM Product_to_categories WHERE product_id = ?", [
      product_id,
    ]);
  ...
  static async get_product_categories(product_id) {
    let q = await db.query(
      "SELECT c.category_id, c.name FROM Product_to_categories AS ptc INNER JOIN Categories AS c ON ptc.category_id = c.category_id WHERE ptc.product_id = ?",
      [product_id]
    );
    return q;
  }
  ...
  static async add_new_order(
    ...
    let lastInsertedOrder = await db.query(
  ...
  static async get_order_info(order_id) {
    let q = await db.query(
      "SELECT o.order_id, o.cost, o.weight AS total_weight, o.address, o.delivery_fee, o.status, o.created_at, oi.product_id, oi.quantity, p.name, p.image_url, p.price, p.weight FROM Orders o JOIN Order_items oi ON o.order_id = oi.order_id JOIN Products p ON oi.product_id = p.product_id WHERE o.order_id = ?;",
      [order_id]
    );
  ...
  static async get_ongoing_order(user_id) {
    let q = await db.query(
      "SELECT o.order_id, o.cost, o.status, o.created_at, p.image_url FROM Orders o JOIN Order_items oi ON o.order_id = oi.order_id JOIN Products p ON oi.product_id = p.product_id WHERE o.status = 0 AND o.user_id = ?",
      [user_id]
    );
  ...
  static async get_onDelivery_order(user_id) {
    let q = await db.query(
      "SELECT o.order_id, o.cost, o.status, o.created_at, p.image_url FROM Orders o JOIN Order_items oi ON o.order_id = oi.order_id JOIN Products p ON oi.product_id = p.product_id WHERE o.status = 1 AND o.user_id = ?",
      [user_id]
    );
  ...
  static async get_order_history(user_id) {
    let q = await db.query(
      "SELECT o.order_id, o.cost, o.status, o.created_at, p.image_url FROM Orders o JOIN Order_items oi ON o.order_id = oi.order_id JOIN Products p ON oi.product_id = p.product_id WHERE o.status = 2 AND o.user_id = ?",
      [user_id]
    );
